...
def compute_summary(scan: ScanResult) -> dict[str, int | float]:
    ...
    return {
        ...
        "http_probes": len(scan.recon.http_probes),
        "crawl_results": len(scan.recon.crawl_results),
        "historical_urls": len(scan.recon.historical_urls),
        "dns_resolutions": len(scan.recon.dns_resolutions),
        "tls_certs": len(scan.recon.tls_certs),
        "fuzz_results": len(scan.analysis.fuzz_results),
